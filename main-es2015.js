...
let UsuarioService = class UsuarioService {
    ...
    login(email, password) {
        ...
        return new Promise(resolve => {
            this.http.post(`${URL}/user/login`, data).subscribe((response) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                if (response['ok']) {
                    yield this.guardarToken(response['token']);
                    resolve(true);
                }
                else {
                    this.token = null;
